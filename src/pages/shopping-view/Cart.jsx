...
// import { addToCart, removeFromCart, clearCart, updateCartItemQuantity } from '@/api/productApi'; // Removed fetchCart
...
//   const { cartItems, refreshCart, loading: cartLoading } = useCart(); // Use cart context
//   const [updating, setUpdating] = useState({});

//   // Removed useEffect for fetching cart since CartProvider handles it
...
//       await refreshCart(); // Refresh cart via context
//       toast({
//         title: "Success",
//         description: `${item.product.title} quantity increased`,
//         className: "bg-green-100 text-green-800 border-green-200",
//       });
//     } catch (err) {
//       toast({
//         title: "Error",
//         description: err.response?.data?.error || "Failed to update quantity.",
//         variant: "destructive",
//       });
//     } finally {
...
//       toast({
//         title: "Success",
//         description: `${item.product.title} quantity decreased`,
//         className: "bg-green-100 text-green-800 border-green-200",
//       });
//     } catch (err) {
//       toast({
...
//   const hasOverQuantityItems = () => {
//   return cartItems.some(item => item.quantity > item.product.stock);
// };
...
//   if (cartLoading) { // Use cartLoading from context
//     return (
...
//             <div
//               key={item.id}
//               className="bg-white rounded-xl shadow-md overflow-hidden transition-all duration-300 border border-gray-100 p-5"
//             >
...
//                   <div>
//                     <Link to={`/shop/product/${item.product.id}`}>
//                       <h3 className="text-lg font-semibold text-gray-800 hover:text-[#E6C692] transition-colors">
//                         {item.product.title}
//                       </h3>
//                     </Link>
//                     <p className="text-gray-600">Category: {item.product.category?.name || 'N/A'}</p>
//                     <p className="text-gray-600">Subcategory: {item.product.subcategory?.name || 'N/A'}</p>
...
// Selected quantity exceeds available stock ({item.product.stock}). Please remove this item and add it again with a valid quantity to proceed.        </div>
//       )}
...
//                     {isUpdating ? <Loader2 className="h-4 w-4 animate-spin inline" /> : item.quantity}
//                   </span>
//                   <button
...
//                   {convertPrice(priceINR * item.quantity)}
//                 </p>
//               </div>
...
//                   disabled={isUpdating}
//                   className={`flex items-center gap-2 px-4 py-2 rounded-full border border-red-500 text-red-500 hover:bg-red-50 transition-all duration-300 ${
...
//       <div className="mt-8 bg-white rounded-xl shadow-md p-6 sticky top-24">
...
//           className={`w-full px-8 py-3 text-black font-medium rounded-full transition-all duration-300 shadow-md flex items-center justify-center gap-2 ${
//             hasOutOfStockItems() || hasOverQuantityItems()
